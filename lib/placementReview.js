...
export const SPEAKING_PROMPT =
  "Speak for 60–90 seconds about a situation where English matters to you. Explain the context, the challenge, what you wanted to achieve, and how the conversation ended.";
...
export const CORE_ITEMS = [
  ...
  { id: "c7", level: "A2", prompt: "You ___ bring your passport to the interview. It is required.", options: ["must", "may", "can", "could"], answer: 0 },
...
export function buildPlacementReview(reviewMeta) {
  ...
  return {
    ...
    speaking: {
      ...
      seconds: Number(meta.speaking?.seconds ?? meta.speakingSeconds ?? placementResult?.raw?.speaking?.seconds ?? 0),
      checks: speakingChecks,
      mode: meta.speaking?.mode || "recording",
      audioDataUrl:
        typeof meta.speaking?.audioDataUrl === "string" ? meta.speaking.audioDataUrl : "",
      audioMimeType:
        typeof meta.speaking?.mimeType === "string" ? meta.speaking.mimeType : "",
    },
    writing: {
      prompt:
        typeof WRITING_TASKS[meta.writingTaskId] === "string"
          ? WRITING_TASKS[meta.writingTaskId]
          : WRITING_TASK,
