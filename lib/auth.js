...
function getResponseMessage(err) {
  ...
  if (data && typeof data === "object") {
    const objectMessage = data.message || data.error;
...
async function request(fn) {
  try {
    ...
    const data = err?.response?.data;
    const msg = getResponseMessage(err);
    const details =
 ...
 */
export async function registerStart(email, locale = "en") {
  return request(() => api.post("/auth/register/start", { email, locale }, opts));
}
