 ...
 */
export async function listSupportTickets({ limit, offset } = {}) {
  const params = new URLSearchParams();
  if (limit) params.append("limit", limit);
  if (offset) params.append("offset", offset);

  return apiFetch(`/api/support/tickets?${params}`, {
    method: "GET",
    headers: {
      "Cache-Control": "no-store",
    },
...
export async function rateSupportTicket({ ticketId, rating, comment }) {
  const token = await getCsrfToken();

  return apiFetch(`/api/support/tickets/${ticketId}/satisfaction`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "CSRF-Token": token,
...
/**
 * Get a short-lived WebSocket auth token for support realtime.
 */
export async function getSupportWebSocketToken() {
  return apiFetch("/api/auth/ws-token", {
    method: "GET",
    headers: {
      "Cache-Control": "no-store",
    },
...
export async function adminUpdateTicketPriority({ ticketId, priority }) {
  const token = await getCsrfToken();

  return apiFetch(`/api/support/admin/tickets/${ticketId}/priority`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      "CSRF-Token": token,
...
export async function adminUpdateTicketTags({ ticketId, tags }) {
  const token = await getCsrfToken();

  return apiFetch(`/api/support/admin/tickets/${ticketId}/tags`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      "CSRF-Token": token,
