...
export function calculatePackagePrice(pkg, countryCode, discountPercent = 0) {
  ...
  let baseEGP = pkg.priceEGP || pkg.priceUSD || 0;

  if (discountPercent > 0) {
    baseEGP = Math.round(baseEGP * (1 - discountPercent / 100));
  }
 ...
 */
export function calculatePerSessionPrice(
  pkg,
  countryCode,
  discountPercent = 0
) {
    ...
    return calculateRegionalPrice(perSession, cc, {
      ...
      egpAmount:
        override.currency === "EGP"
          ...
          : discountPercent > 0
            ? Math.round(
              ((pkg.priceEGP || pkg.priceUSD || 0) / pkg.sessionsPerPack) *
