  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data: payload } = await api.get("/teacher/earnings", {
        params: { status: filter, limit: 100, t: Date.now() },
      });
      setData(payload);
    } catch (error) {
    ...
    <main className="earnings-page">
      ...
      <section className="earnings-summary" aria-label={t(copy, "eyebrow")}>
        <article className="earnings-summary__hero">
          ...
          <div className="earnings-summary__spark" aria-label="Six month earnings trend">{monthlyTrend.map((item) => <i key={item.month} style={{ height: `${Math.max(8, Math.round(((Number(item.totalMinor) || 0) / trendMax) * 100))}%` }} title={`${item.month}: ${formatEGP(item.totalMinor, locale)}`} />)}</div>
        </article>
